...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [[[self.char2id['{']] + [self.char2id[char] for char in word] + [self.char2id['}']] for word in sent] for sent in sents]
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        # previous parts
        word_ids = self.words2charindices(sents)## m_sent*m_word*m_char
        sents_padded_chars = pad_sents_char(sents=word_ids, char_pad_token=self.char2id['<pad>'])
